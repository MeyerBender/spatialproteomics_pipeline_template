...
def plot_overview(dapi_img, metadata):
    ...
    for i, row in metadata.iterrows():
        x, y = (
            row.loc["Centroid X µm"] * magnification,
            row.loc["Centroid Y µm"] * magnification,
        )
        sample_id = row["Name"]
        ...
        ax.hlines(y + h / 2, x - w / 2, x + w / 2)
        ax.text(
            x, y, sample_id, fontsize=8, color="deepskyblue", ha="center", va="center"
        )
...
def plot_images(dapi_img, metadata):
    ...
    plt.tight_layout()
    plt.savefig(
        snakemake.output.individual_cores_plot, bbox_inches="tight", pad_inches=0
    )
